    ...
    def __call__(self, crops, coords):
        ...
        for crop, coord in zip(crops, coords):
            ...
            if torch.rand(1) < self.p:
                ...
                coord_flipped = coord.clone()#不影响coord
                coord_flipped[0] = 1. - coord[2]#因为坐标是归一化后的，1-，即实现了翻转 0 1 2 3
...
class CustomDataAugmentation(object):
    def __init__(self, size=224, min_scale=0.08):
        ...
        '''
        定义两者组合预处理，用于对crop进行进一步图像增强
        '''
        # first global crop
        self.global_transfo1 = transforms.Compose([
            color_jitter,#颜色扭曲
            transforms.RandomApply([GaussianBlur([.1, 2.])], p=1.),#高斯模糊
            normalize,
        ])
        # second global crop
        self.global_transfo2 = transforms.Compose([
            ...
            transforms.RandomApply([Solarize()], p=0.2),#反相
            normalize,
        ])

    def __call__(self, image):
        crops, coords = self.two_crop(image)#得到两张不同crop的view
        crops, coords, flags = self.hflip(crops, coords)#Random翻转
        crops_transformed = []
        crops_transformed.append(self.global_transfo1(crops[0]))#对view1进行进一步增强，高斯模糊的概率为1
        crops_transformed.append(self.global_transfo2(crops[1]))#对view2增强，多了个反相，blur的概率为0.1
        return crops_transformed, coords, flags#crop后的图像；坐标（便于后续逆增强）；翻转标志
